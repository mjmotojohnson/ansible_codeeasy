...
options:
  ...
  uid:
    description:
    - The uid recusively changes the User ID of the cloned volumes files and directies.
  gid:
    description:
    - The gid recusively changes the Group ID of the cloned volumes files and directies.
  junction_path:
    version_added: '2.8'
    description:
...
class NetAppOntapVolumeClone(object):
    ...
    def create_volume_clone(self):
        ...
        if self.uid:
            clone_obj.add_new_child("uid", self.uid)
        if self.gid:
            clone_obj.add_new_child("gid", self.gid)
